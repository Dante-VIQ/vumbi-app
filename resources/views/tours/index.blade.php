...
@section('content')
    <div class="min-h-screen bg-zinc-950 text-white">
        ...
        <div class="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-4 gap-8">
            <!-- Filters Sidebar -->
            <aside x-data="{ open: true }" class="space-y-4">
                <!-- Toggle button (visible only on mobile) -->
                <button @click="open = !open" class="lg:hidden w-full text-left text-lg font-semibold mb-2">
                    Filters {{ '▾' }}
                </button>

                <!-- Filter form (always visible on lg, toggled on mobile) -->
                <div :class="open ? 'flex' : 'hidden'"
                    class="lg:flex flex-col lg:flex-row flex-wrap gap-4 items-end lg:items-center min-w-max">

                    <!-- Destination -->
                    <span class="flex-1 w-auto">
                    ...
                    <span class="flex-1 w-auto">
                        ...
                        <select name="type" form="filter-form"
                            ...
                            @foreach($types as $type)
                                <option value="{{ $type }}" {{ request('type') == $type ? 'selected' : '' }}>{{ ucfirst($type) }}
                                </option>
                            @endforeach
                        </select>
                    </span>
                    ...
                    <button type="submit" form="filter-form"
                        class="shrink-0 bg-green-600 hover:bg-green-500 px-6 py-3 rounded-xl font-medium text-white mt-4 lg:mt-2">
                        Apply Filters
                    </button>
                </div>
            </aside>
