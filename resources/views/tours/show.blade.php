...
<div class="min-h-screen bg-zinc-950 text-white">
    <!-- Hero -->
    <div class="h-96 bg-cover bg-center relative" style="background-image: url('{{ $package->image ?: 'https://picsum.photos/1200/600?random='.$package->id }}')">
        ...
        <div class="absolute bottom-0 left-0 right-0 p-8 max-w-4xl mx-auto">
            <span class="text-xs text-green-400 uppercase tracking-wide bg-zinc-900/80 px-3 py-1 rounded-full">{{ $package->type }}</span>
    ...
    <div class="max-w-4xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-10">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 text-center">
                    <div class="text-2xl font-bold">{{ $package->duration_days }}D / {{ $package->duration_nights }}N</div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <a href="{{ route('tours.show', $similarTour) }}" class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden hover:border-green-400/50 transition">
                        <div class="h-32 bg-cover bg-center" style="background-image: url('{{ $similarTour->image ?: 'https://picsum.photos/200/150?random='.$similarTour->id }}')"></div>
                        <div class="p-3">
                            <h4 class="font-medium text-sm">{{ $similarTour->title }}</h4>
                            <span class="text-green-400 text-sm font-semibold mt-1 block">KSh {{ number_format($similarTour->price) }}</span>
                        </div>
                    </a>
                    @endforeach
                </div>
        ...
        <div class="lg:sticky lg:top-8 h-fit">
            <div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
                ...
                <button onclick="dispatchBookingEvent({{ $package->id }})"
                        class="w-full bg-green-600 hover:bg-green-500 text-white py-3 rounded-xl font-medium mb-4 transition">
                    Book This Safari
                </button>
                <button class="w-full bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 py-3 rounded-xl font-medium transition">
                    <a href="https://wa.me/2547XXXXXXXX?text=I'm%20interested%20in%20{{ urlencode($package->title) }}" target="_blank" class="flex justify-center items-center gap-2">
                        💬 Ask on WhatsApp
                    </a>
                </button>
