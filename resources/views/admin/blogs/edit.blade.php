...
@section('title', 'Edit Blog Post - Vumbi Ventures Admin')
...
<div class="max-w-4xl mx-auto">
    <form action="{{ route('blogs.update', $blog) }}"
                ...
                @foreach($allCategories as $cat)
                    <label class="flex items-center gap-3 p-4 border border-dust-mite cursor-pointer hover:border-sunflare transition">
                        <input type="radio"
                               name="category"
                               value="{{ $cat }}"
                               {{ old('category', $blog->category) == $cat ? 'checked' : '' }}
        ...
        <div class="bg-indigo-night bg-opacity-30 p-6 border border-dust-mite rounded"
             x-data="{
                mediaType: '{{ old('media_type', $blog->media_type ?? 'image') }}',
                currentMedia: '{{ $blog->media_path ? Storage::url($blog->media_path) : null }}',
                currentMediaType: '{{ $blog->media_type }}',
                previewUrl: null,
                removeCurrent: false
             }">
            ...
            <div x-show="currentMedia && !previewUrl && !removeCurrent" class="mb-4">
                <label class="block text-sm text-[#C4B9A6] mb-2">Current Media:</label>
                <div class="relative">
                    <template x-if="currentMediaType === 'image'">
                        <img :src="currentMedia" class="max-h-64 rounded">
                    </template>
                    <template x-if="currentMediaType === 'video'">
                        <video :src="currentMedia" class="max-h-64 rounded" controls></video>
                    </template>

                    <label class="absolute top-2 right-2 bg-red-500 p-2 rounded cursor-pointer">
                        <input type="checkbox"
                               name="remove_media"
                               value="1"
                               x-model="removeCurrent"
                               class="hidden">
                        <i class="fas fa-trash text-raw-linen"></i>
                    </label>
                </div>
                <p x-show="removeCurrent" class="mt-2 text-sm text-red-500">This media will be removed</p>
            </div>

            {{-- File Upload --}}
            <div class="border-2 border-dashed border-dust-mite p-8 text-center"
                 x-show="!currentMedia || previewUrl || removeCurrent"
                ...
                <template x-if="!previewUrl">
                    <div>
                        ...
                        <p class="text-raw-linen mb-2">Drag and drop a new file here, or</p>
                        <button type="button"
        ...
        <div class="bg-indigo-night bg-opacity-30 p-6 border border-dust-mite rounded">
            ...
            <textarea name="description"
                      ...
                      placeholder="Write your blog post content here... (HTML supported)">{{ old('description', $blog->description) }}</textarea>
