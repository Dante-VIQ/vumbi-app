...
<div class="space-y-6">
    ...
    <div class="bg-indigo-night bg-opacity-30 p-6 border border-dust-mite rounded">
        <form method="GET" class="grid md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select name="category" class="w-full bg-transparent border border-dust-mite p-3 text-raw-linen focus:border-sunflare focus:outline-none">
            ...
            <div>
                ...
                <select name="sort_field" class="w-full bg-transparent border border-dust-mite p-3 text-raw-linen focus:border-sunflare focus:outline-none">
            ...
            <div class="flex items-end gap-2">
                <button type="submit" class="bg-terracotta px-6 py-3 text-raw-linen hover:bg-sunflare hover:text-deep-earth transition flex-1">
                    <i class="fas fa-filter mr-2"></i>Filter
                </button>

                <a href="{{ route('blogs.index') }}" class="border border-dust-mite px-6 py-3 text-raw-linen hover:bg-terracotta transition">
    ...
    <div class="flex items-center gap-4 p-4 bg-indigo-night bg-opacity-30 border border-dust-mite rounded"
        ...
        <div class="flex items-center gap-2">
            <input type="checkbox"
                   @change="showBulkActions = $event.target.checked;
        ...
        <div x-show="showBulkActions" x-cloak class="flex gap-2">
            <form action="{{ route('blogs.bulk-destroy') }}" method="POST"
                ...
                <button type="submit" class="border border-red-500 px-4 py-2 text-red-500 hover:bg-red-500 hover:text-raw-linen transition text-sm">
            ...
            <tbody class="divide-y divide-dust-mite">
                ...
                <tr class="hover:bg-indigo-night hover:bg-opacity-30 transition">
                    <td class="p-4">
                        <input type="checkbox"
                               value="{{ $blog->id }}"
                               class="select-item w-4 h-4 bg-transparent border border-dust-mite"
                               @change="if($event.target.checked) { selectedIds.push($event.target.value) }
                    ...
                    <td class="p-4 max-w-xs">
                        ...
                        <p class="text-sm text-[#C4B9A6]">{{ Str::limit(strip_tags($blog->description), 60) }}</p>
                    </td>
                    <td class="p-4">
