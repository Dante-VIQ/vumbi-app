...
<body class="bg-deep-earth text-raw-linen antialiased">

    <div class="flex h-screen overflow-hidden">
        {{-- Sidebar --}}
        <aside
            class="w-64 bg-indigo-night bg-opacity-50 border-r border-dust-mite flex flex-col sidebar-scroll overflow-y-auto">
            {{-- Logo --}}
                ...
                <ul class="space-y-2">
                    ...
                    {{-- <li>
                        <a href="{{ route('admin.categories.index') }}"
                            class="flex items-center gap-3 p-3 rounded transition {{ request()->routeIs('admin.categories.*') ? 'bg-terracotta text-raw-linen' : 'hover:bg-terracotta hover:bg-opacity-30' }}">
                    ...
                    {{-- <li>
                        <a href="{{ route('admin.users.index') }}"
                            class="flex items-center gap-3 p-3 rounded transition {{ request()->routeIs('admin.users.*') ? 'bg-terracotta text-raw-linen' : 'hover:bg-terracotta hover:bg-opacity-30' }}">
                    ...
                    {{-- <li>
                        <a href="{{ route('admin.settings') }}"
                            class="flex items-center gap-3 p-3 rounded transition {{ request()->routeIs('admin.settings') ? 'bg-terracotta text-raw-linen' : 'hover:bg-terracotta hover:bg-opacity-30' }}">
            ...
            <div class="p-4 border-t border-dust-mite">
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-[#C4B9A6]">{{ Auth::user()->email ?? 'admin@vumbi.com' }}</p>
                    </div>
                </div>
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('blogs.index') }}" target="_blank"
                            class="px-4 py-2 border border-dust-mite text-raw-linen hover:bg-terracotta transition rounded text-sm">
